...
# Processing ENSP FASTA sequences to get protein length and list of all ENSPs
# Getting all human genes
gene_lengths <- query("
                        SELECT 
                        ensembl_gene_id,
...
basic_structural_info_clean <- basic_structural_info %>% select(ensg, avg_length, num_proteins, avg_num_exons) %>% na.omit()

write.table(basic_structural_info_clean, "~/Projects/fusion_ML/features/basic_structural_info.csv", sep=",", quote = FALSE, row.names = FALSE)
